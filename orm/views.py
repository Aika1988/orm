...
def get_products():
    ...
    for i in products:
        print(f'{i.name}--{i.price}--{i.amount}--{i.category.name}--{i.category_id}') 

def get_product_by_name(name):
    products = Product.select().where(Product.name==name)# select * from Product where name='product2'
    for i in products:
